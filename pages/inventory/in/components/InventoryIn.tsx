...
const MovementInContent: React.FC = () => {
    ...
    const paginateClick = (value: number, size: number) => {
        const request = {...requestParam};
        request.page = value;
        request.limit = size;
        fetch(request);
    ...
    return (
        <div>
            ...
            <PaginationTable onChange={paginateClick} showSizeChanger defaultCurrent={outs.current_page} defaultPageSize={requestParam.limit} total={outs.total} />
            </div>
        </div>
    );
}

export default MovementInContent;