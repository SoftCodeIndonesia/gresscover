...
const CheckingAdd: React.FC = () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (data.purchase_order_id) {
                    await fetchPurchaseOrderDetails(data.purchase_order_id);
                }

                // Transform items for table (handle children)
                if (data.items && data.purchase_order?.items) {
                    // const items = transformItemsForTable(data.items, data.purchase_order.items);
                    // setitems(items);
                }
            }
        } catch (error) {
    ...
    const handleChildItemChange = (parentKey: string, childKey: string, field: string, value: any) => {
        const parentItem = items.find(item => item.key === parentKey);
        if (!parentItem) return;

        const childIndex = parentItem.children.findIndex(child => child.key === childKey);
        if (childIndex === -1) return;

        const child = parentItem.children[childIndex];
        
        // Prepare updates for child
        let updates: Partial<QualityReportItemFormTmp> = {};
        
        if (field === 'received_quantity') {
            const receivedQty = Number(value) || 0;
            const qtyDefect = child.qty_defect || 0;
            const goodQty = Math.max(0, receivedQty - qtyDefect);
            
            updates = {
                received_quantity: receivedQty,
                good_quantity: goodQty,
                total_value: child.unit_price * receivedQty
            };
        } else if (field === 'qty_defect') {
            const qtyDefect = Number(value) || 0;
            const receivedQty = child.received_quantity || 0;
        ...
        updatedChildren[childIndex] = { ...child, ...updates };
        
        // Recalculate parent totals
        const newReceivedQty = updatedChildren.reduce((sum, child) => sum + child.received_quantity, 0);
        const newDefectQty = updatedChildren.reduce((sum, child) => sum + child.qty_defect, 0);
        const newGoodQty = updatedChildren.reduce((sum, child) => sum + child.good_quantity, 0);
        const newTotalValue = updatedChildren.reduce((sum, child) => sum + child.total_value, 0);
console.log('change', items);
        ...
        console.log('change', updatedItems);
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
                ...
                if(element.is_parent){
                    element.children.forEach(child => {
                        payloadItem.push({
                            ...
                            good_quantity: child.received_quantity - child.qty_defect,
            ...
            let response;
            if (isEditMode) {
                response = await axiosInstance.put(`/quality-reports/${id}`, payload);
            } else {
                response = await axiosInstance.post('/quality-reports', payload);
            }
    ...
    const handleCancel = () => {
        Modal.confirm({
            ...
            okText: 'Ya, Batalkan',
            cancelText: 'Tidak',
            onOk: () => window.location.href = '/checking',
        });
    };
...
const columns: TableColumnsType<QualityReportItemForm> = [
    {
        ...
        render: (_: any, record: QualityReportItemForm, index: number) => {
            ...
            const childIndex = record.child_index;
            ...
            if (isChild && parentIndex !== undefined && childIndex !== undefined) {
                ...
                return (
                    <div style={{ paddingLeft: '32px', display: 'flex', alignItems: 'center' }}>
                        ...
                        >
                            <Input
                                disabled={true}
                                onChange={(e) => handleChildItemChange(items[parentIndex].key, record.key, 'product_name', e.target.value)}
                ...
                // const isExpandable = record.is_parent || record.children.length > 0;
    ...
    },
    {
        ...
        render: (_: any, record: QualityReportItemForm, index: number) => {
            ...
            if (isChild) {
                ...
                const parent = items[record.parent_index];



                return (
                    <Form.Item
                        name={['items', record.parent_index, 'children', record.child_index, 'received_quantity']}
                ...
                return <Tag color="green">{record.received_quantity}</Tag>;
            }
    ...
    },
    {
        ...
        render: (_: any, record: QualityReportItemForm, index: number) => {
            ...
            if (isChild && record.parent_index !== undefined && record.child_index !== undefined) {
                ...
                return (
                    <Form.Item
                        name={['items', record.parent_index, 'children', record.child_index, 'qty_defect']}
                ...
                // Untuk parent item
                if (record.is_parent) {
                    return <Tag color={record.qty_defect > 0 ? 'red' : 'green'}>{record.qty_defect}</Tag>;
                }
                return (
                    <InputNumber
                        min={0}
                        max={record.received_quantity}
                        value={record.qty_defect}
                        onChange={(value) => handleItemChange(record.key!, 'qty_defect', value)}
                        style={{ width: '100%' }}
                    />
                );
            }
    ...
    },
    {
        ...
        render: (_: any, record: QualityReportItemForm, index: number) => {
            ...
            if (isChild && record.parent_index !== undefined && record.child_index !== undefined) {
                ...
                return (
                    <Form.Item
                        name={['items', record.parent_index, 'children', record.child_index, 'defect_type']}
                ...
                return (
                    <Select
                        value={record.defect_type}
                        onChange={(value) => handleItemChange(record.key!, 'defect_type', value)}
                        style={{ width: '100%' }}
                        placeholder="Pilih tipe"
                        allowClear
                        disabled={record.qty_defect === 0}
